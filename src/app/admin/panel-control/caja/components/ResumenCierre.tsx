      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-3 text-center border border-green-200 dark:border-green-800">
            <p className="text-[10px] font-bold text-green-600 dark:text-green-400 uppercase tracking-wide mb-1">Ingresos</p>
            <p className="text-lg font-black text-green-700 dark:text-green-300 font-mono leading-tight">
              {fmt(totales.ingresos_totales)}
            </p>
            <p className="text-[9px] text-green-500 mt-0.5">{totales.turnos_presentes} cobrado{totales.turnos_presentes !== 1 ? 's' : ''}</p>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-3 text-center border border-red-200 dark:border-red-800">
            <p className="text-[10px] font-bold text-red-600 dark:text-red-400 uppercase tracking-wide mb-1">Gastos Día</p>
            <p className="text-lg font-black text-red-700 dark:text-red-300 font-mono leading-tight">
              {fmt(totales.gastos_totales)}
            </p>
            <p className="text-[9px] text-red-500 mt-0.5">{gastos.length} concepto{gastos.length !== 1 ? 's' : ''}</p>
          </div>
          <div className={`rounded-xl p-3 text-center border ${
            ...
            <p className={`text-[9px] mt-0.5 ${totales.ganancia_neta >= 0 ? 'text-emerald-500' : 'text-orange-500'}`}>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700">
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-200 dark:divide-slate-700">
            ...
            <div className="px-3 py-2 text-center">
              <p className="text-[10px] text-slate-400 font-medium">🏦 Transferencia</p>
              <p className="text-sm font-black text-blue-700 dark:text-blue-300 font-mono">{fmt(totales.transferencia)}</p>
            </div>
            <div className="px-3 py-2 text-center">
              <p className="text-[10px] text-slate-400 font-medium">📱 Otro</p>
              <p className="text-sm font-black text-slate-600 dark:text-slate-300 font-mono">{fmt(totales.otro)}</p>
            </div>
          </div>
        </div>

        {/* ── Bloque 3: Clientes del día en 2 columnas ── */}
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 flex justify-between items-center">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {turnosSorted.length === 0 && (
              <p className="text-[11px] text-slate-400 italic text-center py-3">Sin turnos registrados</p>
            )}
            {turnosSorted.map(t => {
              ...
              return (
                <div key={t.id} className={`grid grid-cols-[1fr_auto_auto_auto] gap-x-3 items-center px-3 py-1.5 ${
                  noVino ? 'opacity-50 bg-red-50 dark:bg-red-900/10' : pagado ? 'bg-green-50/50 dark:bg-green-900/10' : ''
                }`}>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-slate-700 dark:text-slate-200 truncate">{t.clienteNombre || '—'}</p>
                    <p className="text-[9px] text-slate-400 truncate">{t.tratamiento || 'sin servicio'}</p>
                  </div>
                  <span className="text-[10px] font-mono text-slate-500 whitespace-nowrap">{fmt(t.monto_total)}</span>
        ...
        {(gastosFijosEmpresa.length > 0 || gastosFijosPersonal.length > 0) && (
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded-xl border border-red-200 dark:border-red-800 overflow-hidden">
              <div className="px-2 py-1 bg-red-50 dark:bg-red-900/20">
                <p className="text-[10px] font-black text-red-600 dark:text-red-400 uppercase tracking-wide">💼 Gastos Empresa</p>
              </div>
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {gastosFijosEmpresa.filter(g => g.activo).length === 0 && (
                  <p className="text-[10px] text-slate-400 italic text-center py-2">Sin gastos</p>
                )}
                {gastosFijosEmpresa.filter(g => g.activo).map(g => (
                  <div key={g.id} className="flex justify-between items-center px-2 py-1">
                    <span className="text-[10px] text-slate-600 dark:text-slate-300 truncate flex-1 mr-1">{g.nombre}</span>
                    {g.pagado ? (
                      <span className="text-[9px] font-black text-green-600 shrink-0">✓ PAGADO</span>
                    ) : g.montoTotal > 0 ? (
                      <span className="text-[10px] font-mono text-red-600 dark:text-red-400 shrink-0 font-bold">
                        -{fmt(g.montoTotal - g.montoAcumulado)}
                      </span>
                    ) : (
                      <span className="text-[9px] text-slate-400 shrink-0">sin monto</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-xl border border-blue-200 dark:border-blue-800 overflow-hidden">
              <div className="px-2 py-1 bg-blue-50 dark:bg-blue-900/20">
                <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-wide">🏠 Gastos Personales</p>
              </div>
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
        ...
        {gastos.length > 0 && (
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-px bg-slate-100 dark:bg-slate-700">
              {gastos.map(g => (
                <div key={g.id} className="flex justify-between items-center px-3 py-1.5 bg-white dark:bg-slate-800">
                  <span className="text-[11px] text-slate-600 dark:text-slate-300 truncate">{g.concepto}</span>
                  <span className="text-[11px] font-mono font-bold text-red-600 dark:text-red-400 ml-2 shrink-0">{fmt(g.monto)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {totalFijosPendiente > 0 && (
          <div className="rounded-xl border-2 border-slate-300 dark:border-slate-600 p-3 bg-slate-50 dark:bg-slate-700/50">
            <div className="grid grid-cols-2 gap-2 text-center">
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Pendiente gastos fijos</p>
                <p className="text-sm font-black text-red-600 dark:text-red-400 font-mono">-{fmt(totalFijosPendiente)}</p>
              </div>
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Ganancia real estimada</p>
                <p className={`text-sm font-black font-mono ${gananciaReal >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {fmt(gananciaReal)}
                </p>
              </div>
            </div>
